...
**Steps 1 & 2: Try your people directory AND Slack in parallel**

Fire both searches concurrently — don't wait for the directory to fail before trying Slack:
...
Slack usergroups (e.g. `@metaflow-dev-group`) are NOT indexed in the people directory — they're Slack-specific constructs. If the directory returns nothing, use the Slack results to discover individual names from channel bot responses, thread authors, or group mentions.

**Step 3: Resolve each member via the people directory (ALL in parallel)**

Once you have individual names (from directory team results or Slack channel discovery), resolve ALL members through the identity cascade in [`_shared/identity-resolution.md`](../_shared/identity-resolution.md) **concurrently** — fire all directory lookups and Sourcegraph `get_contributor_repos` calls in a single parallel batch, not one at a time.
...
|---------|-------------|-----|
| Directory returns 0 for alias | Alias is a Slack usergroup, not a directory team | Try Slack channel search — find the corresponding channel and discover members from recent threads |
