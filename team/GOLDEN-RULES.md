...
- Worker completion: cannot be accepted based on the worker's `SendMessage` alone. Requires the 4-reviewer parallel panel verdicts on the worker's diff per `_shared/parallel-review-panel.md`.
- Verify stage verdict: written by `verify-judge` (opus, separate context) reading the panel review outputs. Coordinator never summarizes into a self-issued verdict.
...
- `team-exec → team-verify` blocked unless every AC has three evidence files (`-red.txt`, `-green.txt`, `-verify.txt`) on disk AND the per-worker parallel panel verdicts on disk.
- `team-verify → team-fix` blocked unless `verify/verdict.md` on disk with `PANEL_VERDICT|failed_fixable`.
...
## 3. 4-Reviewer Parallel Panel on Every Source Modification

**Rule:** Every stage that modifies source files gets a 4-reviewer parallel panel (spec-compliance, code-quality, smoke-test, integration-coherence) per [`_shared/parallel-review-panel.md`](../_shared/parallel-review-panel.md). Four independent agents, separate contexts, separate spawn calls. Not one pass. Not one agent wearing multiple hats.

**Concrete `/team` examples:**
- In `team-exec`: each worker's diff gets `verify/per-worker/{worker}-task-{id}/{lens}-review.md` for all 4 lenses with distinct `agent_id` values.
- In `team-verify`: aggregated diff gets `verify/{lens}-review.md` for all 4 lenses. The stage-level verdict is written by an independent meta-reviewer reading all 4 review files → `verify/panel-verdict.md`.
- In `team-fix`: the fix diff goes back through `team-verify`'s parallel panel from scratch. Previous verify results are discarded (iron-law: fresh evidence every stage).

**Detection at review:** `invariants.parallel_panel_review_enforced` check in STATE.md. Fails if any reviewed diff has fewer than four distinct agent verdicts or if any `agent_id` produced more than one lens.
...
**Concrete `/team` examples:**
- Coordinator writes `handoffs/exec.md` summarizing what workers did. The approval of that handoff (for advancing to verify) is decided by the per-worker parallel panel verdicts already on disk — not by the coordinator reading the handoff and nodding.
- `team-verify → termination = complete`: requires `verify/panel-verdict.md` with `PANEL_VERDICT|approved` authored by `verify-judge`. Coordinator cannot directly set `termination = complete` based on its own reading of defect counts.
...
| "Worker sent `task_complete`; I'll trust the message and skip the parallel panel." | No. The message is a claim, not evidence. The 4-reviewer parallel panel is what turns the claim into evidence. |
| "Spec-compliance found no defects. I'll skip the other panel lenses." | No. The parallel panel is non-negotiable. Each lens finds what the others cannot — smoke-test catches runtime issues, integration-coherence catches cross-module drift. |
| "One reviewer covered multiple lenses; that counts." | No. Same agent, same context = single-lens review. Each lens requires a separate spawn per `_shared/parallel-review-panel.md`. |
