...
- **Python deps** (`defusedxml`, `Pillow`, `openpyxl`) — declared inline in each script via `uv` script metadata; `uv run scripts/...` installs them on first run. No manual `pip install` needed.
