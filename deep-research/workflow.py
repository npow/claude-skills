    ...
    async def run(self, inp: DeepResearchInput) -> str:
        ...
        while round_num < inp.max_rounds and round_num < abs_cap and directions:
            ...
            _researcher_system = (
                ...
                "top-level repo OR monorepo subpath OR specific file/blob/tree URL — you "
                "MUST verify it resolves before including it, using whichever code-search "
                "tool is available (e.g. an MCP code-search server's repo-list or "
                "list-files endpoint). If the existence-check returns zero matches → the "
                "path is invalid; do not cite it. A grep/keyword hit proves a pattern "
                "matched somewhere — it is NOT evidence that the directory/path you "
                "extrapolated to actually exists. The directory-existence check is a "
                "separate call.\n"
                ...
                "own M flows per query on table X'), state explicitly what the data "
                "source DOES and DOES NOT include. Tables that index scheduled workflows "
                "typically miss ad-hoc or interactive usage; cost-aggregation tables "
                "typically miss compute that bypasses the standard scheduler; ownership "
                "snapshots typically lag transfers and reorgs. If you assert a coverage "
                "claim, identify the source's scope limits and, where possible, search "
                "for an alternative source that covers the gap before reporting the "
                "number as comprehensive.\n"
                "STRUCTURED_OUTPUT_START\n"
        ...
        if all_claims:
            ...
            verifier_output = await _spawn(
                ...
                system_prompt=(
                    ...
                    "Extract all cited code/repo URLs from the findings (top-level repos "
                    "AND monorepo subpaths). For a stratified sample of up to 20 URLs, "
                    "call whichever code-search tool's existence-check endpoint is "
                    "available — list-repos for top-level, list-files for subpaths. "
                    "Record in UNVERIFIABLE any URL that returns zero matches or an "
                    "empty-listing signal — those are broken citations and the report "
                    "must NOT present them as valid evidence. Budget is bounded by "
                    "parallelism; 20 calls in parallel typically complete in tens of "
                    "seconds. "
                    "STRUCTURED_OUTPUT_START\n"
                    "VERIFIED|<json array>\n"
        ...
        # blast radius (a reader cannot tell whether a topic is absent because
        # evidence shows nothing, or absent because no agent looked).
        ...
        synth_result = await _spawn(
            ...
            system_prompt=(
                ...
                "distinguish managers from ICs explicitly. A person is a MANAGER only when "
                "the evidence is a formal org-hierarchy data source — an ownership table, "
                "HR system, org-chart doc, or management-chain query — that places them "
                "above at least one direct report. Code ownership entries, commit "
                "authorship, chat mentions, and 'lead engineer on X' phrasing are NOT "
                "manager evidence; those people are ICs unless an explicit 'Manager of X' "
                "claim exists. NEVER place an IC in a column labeled 'Manager' — use "
                "'Lead / IC' or annotate `(IC)`. If role evidence is missing, write "
                "`*(role unknown)*` — do not guess from proximity, list position, or quote "
                "frequency. "
                ...
                "the report's blast radius — a reader cannot tell whether a topic is absent "
                "because evidence shows nothing, or absent because no agent looked. "
                ...
                "Example pattern: a tool described as 'not the standard' in one source while "
                "another source documents many teams using it in production — both views "
                "stay in the report, not one. "
                ...
                "footnote, parenthetical, or 'as of {date}' phrase). For attribution claims "
                "sourced from material >12 months old, append `[STALE: source from {date}]` "
                "inline. This is required for fast-moving topics like org charts, platform "
                "inventories, and tool adoption — multi-month-old org-doc snapshots are "
                "routinely wrong by report time. "
                ...
                "every 'Manager' column entry and confirm it has formal-hierarchy evidence "
                "or an `(IC)` / `*(role unknown)*` annotation; scan for every verbatim quote "
