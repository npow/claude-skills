...
- TESTABILITY (empirical — mutation testing): "**Mutation testing lane (OPTIONAL, spawned when tests exist for target files).** Invoke the `mutation-test` skill as a background agent with a 3-minute total budget. Target: changed files (in `--diff` mode) or the primary artifact files (in normal mode). The agent generates semantically meaningful mutations (business logic inversions, boundary changes, error path removals), runs the test suite against each, and reports surviving mutants. Map results into the defect registry: surviving `business_logic` or `concurrency` mutations → critical testability defects; surviving `data_flow` or `error_handling` mutations → major testability defects. Include the mutation-test report as an appendix in the final QA report. Skip this lane if: no test files found for target, `--no-cross-model` flag is set (budget-conscious mode), or artifact type is not `code`."
