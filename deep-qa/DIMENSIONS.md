...
- INTERNAL CONSISTENCY: "Do terms in the glossary/Section 1 match usage in later sections?", "Does the requirements list conflict with the stated constraints?", "Do the examples match the rules they illustrate?", "For EVERY rule/gate/constraint the spec defines: does ANY other section (examples, defaults, exception clauses, state schemas, escape-hatch language, stall recoveries) permit, demonstrate, or enable the exact behavior the rule bans? Check every example value, every default, every 'ONLY exception', every 'if not done', every 'or track' — these are where rules get silently undermined.", "Do state file schemas, data structures, and enum values include states or transitions that the spec's rules forbid? (e.g., a 'dismissed' status in a schema when the rules say nothing can be dismissed)"
...
- "internal_consistency × edge_cases" (rule-vs-content) — "Treat the spec's own rules as a contract and its examples/defaults/schemas as an implementation. Audit the implementation against the contract. For each rule that says 'never X' or 'always Y': grep the rest of the spec for content that does X or skips Y. Examples that demonstrate banned patterns, defaults that create escape hatches, exception clauses that swallow the rule, state schemas with forbidden states, recovery instructions that permit the stall they're supposed to prevent — all are internal-consistency defects. This is the highest-yield angle for specs that define enforcement rules, because the spec author's own examples are the most trusted source of behavior and the most likely place for unconscious rule violations."
...
- TESTABILITY (empirical — mutation testing): "**Mutation testing lane (OPTIONAL, spawned when tests exist for target files).** Invoke the `mutation-test` skill as a background agent with a 3-minute total budget. Target: changed files (in `--diff` mode) or the primary artifact files (in normal mode). Consume structured result records — only `survived` status is a test gap (not `killed`, `timeout`, `skipped_budget`, `baseline_failed`, `no_tests`). Apply scope-aware severity: `survived` + `scope: changed` + category `business_logic`/`concurrency` → critical; other changed-scope survivors → major; `adjacent_unchanged` → minor (pre-existing gap, informational); `unrelated` → suppress. Skip this lane if: no test files found, `--no-cross-model` flag is set, or artifact type is not `code`."
...
**Typical angles (new dimensions):**
- OBSERVABILITY: "For each error path, does the error message + log context provide enough information for an operator who has never seen this code to distinguish between the 3 most likely root causes?", "Are structured logging fields present for request tracing, or are errors logged as opaque strings?", "When this operation times out, what diagnostic information is available to determine WHERE in the call chain it stalled?", "Is the logging framework actually wired to emit output? In Python: `logging.getLogger(__name__)` without `basicConfig()` or explicit handler attachment produces zero output — the code LOOKS instrumented but logs go to `/dev/null`. In Java: loggers without appenders. In Go: `log.Logger` writing to a discarded writer. Grep for logger creation and verify a handler/appender/writer is configured in the process entry point. This is the 'configured but unconfigured' pattern — the most dangerous observability gap because it passes code review ('logging is in place') while producing zero operational evidence when the process crashes.", "For long-running daemon processes: is fatal crash evidence captured before the process exits? A bare `asyncio.run(main())` or `worker.run()` with no top-level try/except means a crash produces zero log output if the logging framework depends on the process staying alive. Wrap the outermost entry point in try/except that logs at CRITICAL level with full traceback."
- DEGRADED MODE: "For each external dependency (DB, cache, queue, upstream service), what is the specified behavior when it is persistently unavailable while all other dependencies function normally?", "Do partial-failure states produce correct results, stale-but-safe results, or silently wrong results?", "Are circuit breakers or fallback paths defined, or does a single dependency failure cascade to total service unavailability?", "Process self-supervision: for every long-running daemon or worker process, what supervises it? If it crashes, who restarts it? A bare `nohup ... &` with no systemd/s6/supervisord/health-check means a silent death leaves the system degraded with zero recovery. Check for: (a) process supervisor or restart-on-crash mechanism, (b) liveness probe or heartbeat that detects death, (c) crash logging that captures evidence before exit. 'ensure_running()' at launch time is not supervision — it only checks at spawn, not continuously."
...
- RESOURCE EXHAUSTION: "For every subprocess.run/Popen call: is there a timeout= parameter? What happens if the subprocess hangs?", "For every retry loop: is there a max-retry cap? What happens if retries are exhausted?", "For every network call (HTTP, S3, gRPC): is there a timeout? What happens on network partition — does the caller hang indefinitely?", "For every file/connection open: is it closed on all exit paths including exceptions? grep for open()/connect() without context managers.", "Shared pool starvation (priority inversion): when heterogeneous workloads share a bounded resource pool (thread pool, connection pool, worker activity slots, database connection limit), can slow operations monopolize the pool and starve fast ones? A 2-hour research task and a 10-second file-write task sharing the same N-slot pool means the writes can queue indefinitely when N long tasks are running. Check for: (a) pool partitioning or reserved capacity for priority classes, (b) separate queues for fast utility operations vs slow batch operations, (c) timeouts that account for queue wait time, not just execution time.", "Timeout coverage gap: does the timeout parameter cover the full latency path or only execution time? In task queues and connection pools, `execution_timeout` only starts counting after the task gets a slot. If the task waits 5 minutes in the queue, a 10s execution timeout is still 10s — but the caller experienced 5m10s of wall time. Check for `schedule_to_start_timeout` (Temporal), `connection_timeout` vs `socket_timeout` (HTTP), `acquire_timeout` (connection pools) — any system where 'getting a slot' and 'using the slot' are separate phases needs separate timeouts for each."
...
- "temporal_coupling × correctness" — "When a PR changes files composed by an orchestrator NOT in the diff (bootstrap sequence, pipeline, command list builder), each file may be correct in isolation but the orchestrator's execution order creates a happens-before violation. Open the orchestrator and trace: does cleanup/setup in file A complete before the subprocess/consumer in file B runs? This is the 'file-local review without call-site composition' class of bug — invisible to per-file review, only surfaces when you trace the caller's sequencing."
...
- "observability × degraded_mode" — "When the process itself dies (not a dependency — the process), is there any evidence? A daemon with `logging.getLogger()` but no `basicConfig()` and no top-level crash handler produces zero bytes on fatal exit. The combination of unconfigured logging + no supervisor = silent death with no recovery and no evidence."
- "resource_exhaustion × concurrency" — "Priority inversion in shared pools: when fast utility operations and slow batch operations share a bounded pool (activity slots, thread pool, connection pool), the slow ones fill all slots and the fast ones starve. This is a concurrency failure (scheduling fairness) manifesting as resource exhaustion (timeout). Neither dimension catches it alone — CONCURRENCY looks at thread safety, RESOURCE_EXHAUSTION looks at missing timeouts. The fix is pool partitioning or separate queues."
...
- PROVENANCE:
  ...
  - "**Cross-platform identity.** A subject may have different handles on different systems: GHE login, OSS GitHub login, Slack handle, email username, Pandora ID, ticket assignee field. Build the identity map once (e.g., `jdoe` on GitHub Enterprise = `janedoe` on public GitHub = `jane.doe@example.com` in Jira) and use it in every re-query. A report that hardcodes one handle in all queries will silently miss attributions on other platforms (e.g., OSS PRs authored under a personal handle won't return for the corp handle). When the source system can return zero results for a valid claim because of a handle mismatch, that's a re-query defect, not a missing-attribution defect — but the report's claim is still untrustworthy until the strict predicate succeeds under the right handle." # oss-ok
