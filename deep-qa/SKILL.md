...
- "**Subprocess / entrypoint re-entry invariant audit (CRITICAL priority for PRs that change the invocation contract of a primary entrypoint).** If the PR adds a new required argv position, env var, or flag that every subprocess re-entering the entrypoint must carry (e.g. a flow name, a selector, a config path), enumerate EVERY command-builder in the repo that re-enters the entrypoint and verify each one respects the new invariant. High-yield grep patterns: `sys.executable`, `script_name`, `os.path.basename(sys.argv[0])`, `entrypoint = [`, `_cmd(`, `_build_command`, `construct_*_command`, `subprocess.Popen`, `spm.run_command`. These builders are typically distributed across many plugins — orchestrators (Airflow, Argo, Step Functions), task runners (Kubernetes, Batch), monitor sidecars, card renderers, test harnesses. Refactors commonly update the 2–3 most obvious Runner/CLI paths and miss the rest, which breaks remote execution even when local tests pass. Report each unupdated command builder as a separate defect."
- "**Cross-language consumer audit (CRITICAL priority for PRs that change any data contract).** For every data-shape change (dict keys, artifact schema, API payload shape, field name, enum value), grep the ENTIRE repo — including non-Python files: `.svelte`, `.tsx`, `.ts`, `.jsx`, `.js`, `.vue`, `.html`, `.sql`, `.proto`, `.graphql`, `.json`, `.yaml`, `.yml`, `.hbs`, `.mustache` — for consumers of the OLD shape. Metaflow-class systems frequently have Svelte/React UIs consuming Python-serialized dicts; schema migrations frequently have SQL views referencing old column names; API changes frequently have TypeScript type definitions or proto files that weren't updated. A data-contract-only Python PR is incomplete if non-Python consumers still read the old shape. Report every consumer of the old shape separately."
...
| "I updated the obvious subprocess entrypoints (Runner, Deployer) — good enough" | Metaflow-class systems have command-builders distributed across 10+ plugins (orchestrators, task runners, monitor sidecars, card renderers, CLI wrappers). Grep for `script_name = os.path.basename(sys.argv[0])`, `entrypoint = [`, `sys.executable`, and similar patterns across the ENTIRE repo and verify every one respects the new invocation invariant. Remote execution paths break silently when a subset is updated. |
| "This is a Python PR — I only need to check Python files" | Data-contract changes affect ALL consumers regardless of language. Grep non-Python files: `.svelte`, `.tsx`, `.ts`, `.js`, `.vue`, `.sql`, `.proto`, `.graphql`, `.json`, `.yaml`. Frontend UI code, TypeScript types, GraphQL schemas, SQL views, and proto files routinely consume Python-serialized shapes and break silently on schema changes. A Python-only audit of a data-contract change is incomplete by construction. |
| "The registry is scoped to my module" (for metaclass/decorator-populated global registries) | Python class definitions populate class-level / metaclass-level state at IMPORT time, not at entrypoint time. Any transitive import (including test fixtures, documentation examples, helper flows) populates the registry. Construct the concrete import-leak scenario: `file_b.py: from file_a import OtherFlow; class MyFlow(FlowSpec): ...; FlowSpec.main()` — the registry contains BOTH flows. Duplicate class names across modules silently overwrite. |
